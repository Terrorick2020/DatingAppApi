...
export class LikeService {
  ...
  async createLike(dto: CreateLikeDto) {
    try {
      ...
      const fromUserResponse = await this.userService.findByTelegramId(dto.fromUserId)
      const toUserResponse = await this.userService.findByTelegramId(dto.toUserId)

      if (!fromUserResponse.success || !toUserResponse.success || !fromUserResponse.data || !toUserResponse.data) {
        this.logger.warn(
      ...
      if (reverseLike) {
        ...
        const chatCreationResult = await this.createChatForMatch(dto.fromUserId, dto.toUserId)
        ...
        case 'matches':
          ...
          if (likes.length > 0) {
            for (const like of likes) {
              ...
              const otherUserId = like.fromUserId === telegramId ? like.toUserId : like.fromUserId;
              
              // Проверяем существование чата в Redis
              const chatId = await this.findChatBetweenUsers(telegramId, otherUserId);
  ...
  private async findChatBetweenUsers(user1Id: string, user2Id: string): Promise<string | null> {
    try {
      // Получаем список чатов для первого пользователя
      const userChatsResponse = await this.chatsService.findAll({ telegramId: user1Id });
      
      if (!userChatsResponse.success || !userChatsResponse.data || !Array.isArray(userChatsResponse.data)) {
        return null;
      }
      
      // Ищем чат со вторым пользователем
      const chatWithUser = userChatsResponse.data.find(chat => 
        chat.toUser && chat.toUser.id === user2Id
      );
