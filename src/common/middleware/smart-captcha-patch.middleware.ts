...
@Injectable()
export class SmartCaptchaPatchMiddleware implements NestMiddleware {
	private readonly CONTEXT = 'SmartCaptchaPatchMiddleware'
	...
	async use(req: Request, res: Response, next: NextFunction) {
		try {
			...
			if (req.method !== 'PATCH') {
				this.logger.debug(
					`Запрос ${req.method} ${req.path} не является PATCH - пропускаем`,
					this.CONTEXT
				)
				return next()
			}
			...
			if (!captchaToken) {
				this.logger.debug(
					`PATCH запрос ${req.path} без заголовка X-Captcha-Token - пропускаем`,
					this.CONTEXT
				)
				return next()
			}
			...
			this.logger.debug(
				`Обнаружен заголовок X-Captcha-Token в PATCH запросе ${req.path}`,
				this.CONTEXT
			)
			...
			this.logger.error(
				`Ошибка в SmartCaptchaPatchMiddleware: ${error?.message || 'Unknown error'}`,
				error?.stack,
				this.CONTEXT
			)
